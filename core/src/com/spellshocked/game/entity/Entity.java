    ...
    public void periodic() {
        ...
        if (camera != null && ortCam != null) {
            ...
            if (abs(ortCam.zoom - 0.5) <= DEADZONE){
                camera.position.set(clamp(getX(), xMin + 100, xMax - 85), clamp(absY, yMin + 30, yMax - 30) + currentTileZ*16, camera.position.z); //zoom == 0.5
            }
            else if (abs(ortCam.zoom - 1) <= DEADZONE){
                camera.position.set(clamp(getX(), xMin + 200, xMax - 185), clamp(absY, yMin + 110 - currentTileZ, yMax - 100 - currentTileZ) + currentTileZ*16, camera.position.z); //zoom == 1
            }
            else if (abs(ortCam.zoom - 1.5) <= DEADZONE){
                camera.position.set(clamp(getX(), xMin + 300, xMax - 285), clamp(absY, yMin + 150, yMax - 150) + currentTileZ*16, camera.position.z); //zoom == 1.5
            }
            else if (abs(ortCam.zoom - 2) <= DEADZONE){
                camera.position.set(clamp(getX(), xMin + 400, xMax - 385), clamp(absY, yMin + 220, yMax - 220) + currentTileZ*16, camera.position.z); //zoom == 2
            }
            else {
